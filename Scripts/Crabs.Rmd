...
ggplot(heat_df, aes(x = comp1, y = comp2, fill = signfi)) + geom_tile() + facet_grid(Period~Temperature) + ggtitle("Differences between Week Number Within Periods for Green Crabs")
...
ggplot(heat_df, aes(x = comp1, y = comp2, fill = signfi)) + geom_tile() + facet_grid(Period~Temperature) + ggtitle("Differences between Week Number Within Periods for Hemis")
...
ggplot(heat_df, aes(x = comp1, y = comp2, fill = signfi)) + geom_tile() + facet_grid(Period~Temperature) + ggtitle("Differences between Week Number Within Periods for Rock crabs")
...
ggplot(period_diff, aes(x = comp1, y = comp2, fill = significant)) + geom_tile() + facet_grid(Temperature~Species) + ggtitle("Differences between period 1 and period 2 within each temperature regime for each species for the final week of feeding")
```
...
subcensor <- subset(finalEatCensor, finalEatCensor$proportion.eaten > 0)

pos_model2 <- glmmTMB(
  proportion.eaten ~ Species.x*as.factor(Temperature) + 
                (1 | ID) + (1 | runID),
  family = Gamma(link = "inverse"),
  data = data, na.action = "na.fail"
)
...
pos_model3 <- glmmTMB(
  proportion.eaten ~ Species.x*as.factor(Temperature) + as.factor(Period)  + 
                (1 | ID) + (1 | runID),
  family = Gamma(link = "inverse"),
  data = data, na.action = "na.fail"
)
...
ggplot(aes(x = as.factor(Temperature), y = proportion.eaten, fill = Species.x), data = subcensor) + geom_boxplot() + facet_wrap(~Species.x) + scale_fill_manual(values = c("CI" = "darkorange", "CM" = "darkgreen", "HS" = "darkred"))

# Simulate DHARMa residuals
sim_res_pos <- simulateResiduals(fittedModel = pos_model, n = 1000)
sim_res_pos2 <- simulateResiduals(fittedModel = pos_model2, n = 1000)
sim_res_pos3 <- simulateResiduals(fittedModel = pos_model3, n = 1000)
...
testDispersion(sim_res_pos2)
testZeroInflation(sim_res_pos2)  # should be low for positive model
testDispersion(sim_res_pos3)
testZeroInflation(sim_res_pos3)  # should be low for positive model


zero_model <- glmmTMB(
  binaryfed ~ Species.x*Temperature + Period + 
                  (1 | ID) + (1 | runID),
  family = binomial,
  data = finalEatCensor
)
...
ggplot(finalEatCensor, aes(x = as.factor(Temperature), fill = as.factor(binaryfed))) +
  facet_wrap(~Species.x) + geom_bar() + scale_fill_manual(values = c("0" = "darkred", "1" = "darkgreen"))
...
```{R, include = FALSE, echo=FALSE,fig.keep='all', fig.width=7, fig.height=5, warning = FALSE}
#Rock crab - censor model -----------------------------------------------------
...
all.ZIglm <- glmmTMB(proportion.eaten ~ Temperature*Period*WeekNum, data = censorsub, ziformula = ~Temperature, na.action = "na.fail")
#dredge(all.ZIglm)
...
all.glm <- glm(proportion.eaten ~ Temperature*Period*WeekNum, data = censorsub, na.action = "na.fail")
#dredge(all.glm)
