...
ggplot(mastergraph, aes(x = Temperature, y = dead, color = Species)) + geom_point(position = position_jitter(width = 0.2)) +
  geom_smooth(aes(group = 1), method = "lm", se = FALSE) +
  xlab("Temperature (°C)")+
  ylab("Binary Indicating Dead (0) or Alive (1)")+
  ...
  legend.box.margin = margin(t = 5, r = 5, b = 5, l = 5, unit = "mm")
) +
  scale_x_continuous(breaks = c(18, 20, 22, 24, 26)) +
  scale_y_discrete(breaks = c(0,1))
  

dat <- subset(mastergraph, mastergraph$Species == "Irroratus" & mastergraph$Temperature == 18 & mastergraph$dead == 0)
nrow(dat)
...
ggplot(aes(x = as.factor(Temperature), y = (proportion.eaten), fill = Species.x), data = subcensor) + 
  geom_boxplot() + 
...
#need to fix zero model and write in results summary in document
#also need to review what you wrote and make corrections
#make comparisons with Cheng et al 2014
...
dat <- subset(finalEatCensor, finalEatCensor$Species.x == "HS" & finalEatCensor$binaryfed == 0)
nrow(dat)
...
pred_df <- expand.grid(
  ...
  Period = 1  # pick a period to hold constant
)
...
ggplot(pred_df, aes(x = Temperature, y = proportion_eaten, color = Species.x)) +
  ...
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5))+
  xlab("Temperature (°C)")+
  ylab("Predicted Proportion (% Body Weight) Eaten") 
