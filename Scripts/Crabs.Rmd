...
ggplot(finalEatCensor, aes(x = Temperature, y = binaryfed, color = Species.x)) +
  ...
  scale_color_manual(values = c("CM" = "darkgreen",
                                "HS" = "darkred",
                                "CI" = "darkorange"))
...
#Amount Eaten with log_mass_bs and squared --------------------------------------------------------------------
library(DHARMa)
...
best.mod <- glm(amount.eaten ~ Species.x*Temperature + log_mass_ws + temp_squared, family = Gamma(link = "identity"), data = data, na.action = "na.fail")
summary(best.mod)
sim_res_pos <- simulateResiduals(fittedModel = best.mod, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
testZeroInflation(sim_res_pos)  # should be low for positive model
testOutliers(sim_res_pos)
...
all.mod2 <- glm(proportion.eaten ~ Species.x*Temperature*as.factor(Period) + log_mass_bs + log_mass_ws, family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod2, rank = "AICc")

#best model, compared to Gaussian and link = identity (looks okay...)
best.mod2 <- glm(proportion.eaten ~ Species.x*Temperature + log_mass_ws, family = Gamma(link = "log"), data = data, na.action = "na.fail")
summary(best.mod2)
...
#Proportion Eaten With Temp Squared----------------------------------------------------------------

# Hurdle model: zeros vs positives (binomial) + positive consumption (Gamma)
all.mod <- glm(proportion.eaten ~ Species.x*Temperature + log_mass_ws + temp_squared, family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod, rank = "AICc")

#Gamma log and identity look similar in terms of diagonistc plots, plots look "ok"
best.mod <- glm(proportion.eaten ~ Species.x*Temperature + temp_squared, family = Gamma(link = "identity"), data = data, na.action = "na.fail")
summary(best.mod)
...
#PROPORTION EATEN  WITH TEMPERATURE SQUARED ----------------------------------
...
#model diagonistics look "ok" some outliers...
sim_res_pos <- simulateResiduals(fittedModel = pos_model2, n = 1000)
testDispersion(sim_res_pos)
testZeroInflation(sim_res_pos)  # should be low for positive model
testOutliers(sim_res_pos)
...
# ---- Predictions for Rock Crab (pos_model2_rock) ----
temp_seq_rock <- seq(18, 26, length.out = 100)

pred_rock <- data.frame(
  Temperature = temp_seq_rock,
  temp_squared = temp_seq_rock^2,
  Species.x = "CI"
)

pred_link_rock <- predict(pos_model2_rock, newdata = pred_rock, se.fit = TRUE)
pred_rock$fit      <- exp(pred_link_rock$fit)
pred_rock$lwr      <- exp(pred_link_rock$fit - 1.96 * pred_link_rock$se.fit)
pred_rock$upr      <- exp(pred_link_rock$fit + 1.96 * pred_link_rock$se.fit)

# ---- Predictions for CM and HS (pos_model2_rest) ----
temp_seq_rest <- seq(18, 26, length.out = 100)

pred_rest <- expand.grid(
  Temperature = temp_seq_rest,
  Species.x = c("CM", "HS")
)
pred_rest$temp_squared <- pred_rest$Temperature^2

pred_link_rest <- predict(pos_model2_rest, newdata = pred_rest, se.fit = TRUE)
pred_rest$fit <- exp(pred_link_rest$fit)
pred_rest$lwr <- exp(pred_link_rest$fit - 1.96 * pred_link_rest$se.fit)
pred_rest$upr <- exp(pred_link_rest$fit + 1.96 * pred_link_rest$se.fit)

# ---- Combine predictions ----
pred_all <- rbind(pred_rock, pred_rest)

# ---- Raw data for both datasets ----
raw_rock <- subset(finalEatCensor, proportion.eaten > 0 & Species.x == "CI")
raw_rest <- subset(finalEatCensor, proportion.eaten > 0 & Species.x %in% c("CM", "HS"))
raw_all  <- rbind(raw_rock, raw_rest)

# ---- Plot ----
ggplot() +
  geom_point(data = raw_all,
             aes(x = Temperature, y = proportion.eaten, color = Species.x),
  ...
  geom_line(data = pred_all,
            ...
            linewidth = 1.2) +
  scale_color_manual(
    limits = c("HS", "CM", "CI"),  # controls legend order
    values = c("CI" = "darkorange", "CM" = "darkgreen", "HS" = "darkred"),
    labels = c("CI" = "Rock Crab", "CM" = "Green Crab", "HS" = "Asian Shore Crab"),
    name = "Species"
  ) +
  ...
  scale_x_continuous(breaks = c(18, 20, 22, 24, 26)) +
  xlab("Temperature (°C)") +
  ylab("Log Proportion Eaten (Amount Eaten over Wet Weight") +
  ggtitle("Predicted Proportion Eaten Across Temperature") +
  theme_classic() +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5))+
