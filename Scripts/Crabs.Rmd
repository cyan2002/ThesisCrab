...
That model doesn't really describe that the hemigrapsus and green crab rate is 0. The general temperature variable is across the whole dataset and not species specific.
I can tell based off the raw data that there are only two death points for green crabs and zero for hemigrapsus. Therefore it might not be necessary to model them. Instead, here I'll just model rock crabs.
...
I'll first model all the data in the censor model (all species)
...
cat("Best zero-inflated model for CI")
plot(ZIglm.resids)
...
par(mfrow = c(2,2))
plot(best.mod1, main = "Best GLM model for CI")
#plot(best.mod2)
#plot(best.mod3)
...
Next is green crabs. I dredged the full model then compared all the top models within 2 delta units of the top across Gamma, 
...
par(mfrow = c(2,2))
plot(mod1, main = "Best GLM model for CM")
...
par(mfrow = c(2,2))
plot(all.mod.mixed3, main = "Diagnostic plots for shore crabs")
#plot(all.mod.mixed6)
