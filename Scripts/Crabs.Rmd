...
pos_model <- glmmTMB(
  proportion.eaten ~ Species.x*Temperature + as.factor(Period) + 
                (1 | ID) + (1 | runID),
...
zero_model <- glmmTMB(
  binaryfed ~ Species.x*Temperature + as.factor(Period) + 
                  (1 | ID) + (1 | runID),
  family = binomial,
  data = finalEatCensor
)
...
#SOLO ROCK CRAB MODEL
pos_model2 <- glm(
  proportion.eaten ~ Temperature + temp_squared,
...
#in a Gamma inverse model, the slope is not constant, changes
...
pred_df <- expand.grid(
  ...
  Period = as.factor(1)  # pick a period to hold constant
)

pred_link <- predict(pos_model, newdata = pred_df, type = "link", se.fit = TRUE, re.form = NA)

pred_df <- pred_df %>%
  mutate(
    fit_link = pred_link$fit,
    se_link = pred_link$se.fit,
    proportion_eaten = 1 / fit_link,
    lower = 1 / (fit_link + 1.96 * se_link),  # note: bounds flip because 1/x is decreasing
    upper = 1 / (fit_link - 1.96 * se_link)
  )

ggplot(pred_df, aes(x = Temperature, y = proportion_eaten, color = Species.x)) +
  geom_point(data = data, aes(x = Temperature, y = proportion.eaten, color = Species.x),
             position = position_jitter(width = 0.1), alpha = 0.5, inherit.aes = FALSE) +
  geom_line(linewidth = 1.2) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = Species.x), alpha = 0.2, color = NA) +
  theme_classic() +
  labs(color = "Species") +
  ggtitle("Predicted Proportion Eaten Across Temperature") +
  scale_color_manual(limits = c("HS", "CM", "CI"),
                     values = c("CI" = "darkorange", "CM" = "darkgreen", "HS" = "darkred"),
                     labels = c("CI" = "Rock Crab", "CM" = "Green Crab", "HS" = "Asian Shore Crab")) +
  scale_fill_manual(values = c("CI" = "darkorange", "CM" = "darkgreen", "HS" = "darkred")) +
  guides(fill = "none") +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5)) +
  xlab("Temperature (°C)") +
  ylab("Predicted Proportion (% Body Weight) Eaten")+scale_y_continuous(trans = "log10")

#Rock crab prediction looks different from what we would expect (curve is different, try doing only rock crabs.)

temp_seq <- seq(min(data2$Temperature), max(data2$Temperature), length.out = 100)

# new data frame for predictions
pred_df <- data.frame(
  Temperature = temp_seq,
  temp_squared = temp_seq^2
)

pred_link <- predict(pos_model2, newdata = pred_df, se.fit = TRUE, re.form = NA)
...
pred_df <- pred_df %>%
  mutate(
    ...
    lower = fit - 1.96 * se,
    upper = fit + 1.96 * se,
    proportion_eaten = fit
  )

ggplot(aes(x = Temperature, y = proportion.eaten), data = data2) +
  geom_point() +
  geom_ribbon(data = pred_df, aes(x = Temperature, ymin = lower, ymax = upper),
              alpha = 0.2, inherit.aes = FALSE) +
  geom_line(data = pred_df, aes(x = Temperature, y = proportion_eaten),
            linewidth = 1.2, inherit.aes = FALSE) +
  theme_classic() +
  xlab("Temperature (°C)") +
  ylab("Proportion Eaten") + 
  ggtitle("Predicted Amount Eaten for Atlantic Rock Crabs")
