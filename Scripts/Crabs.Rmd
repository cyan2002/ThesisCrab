...
gam_model <- gam(
  amount.eaten.log ~ s(Temperature, by = Species.x, bs = "tp", k = 5)
               + s(log_mass_ws, by = Species.x, bs = "tp", k = 10)
               + Species.x
               + s(ID, bs = "re"),
  data = data,
  method = "REML",
...
newdata <- expand.grid(
  ...
  Species.x   = c("CI", "CM", "HS"),
  log_mass_ws = 0  # centered around mean, so 0 = mean
)

preds <- predict(gam_model, newdata = newdata, se.fit = TRUE)
...
ggplot(newdata, aes(x = Temperature, y = fit, color = Species.x, fill = Species.x)) +
