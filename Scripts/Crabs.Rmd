...
summary(glm(dead ~ Species + Trial, family = binomial(link = "logit"), data = master))

#model fails due to not enough data points

subdata <- subset(master, master$Species == "Irroratus")

mod <- glm(dead ~ Trial, family = binomial, data = subdata)
summary(mod)
```
