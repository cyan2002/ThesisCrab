...
Does Size have an effect on consumption?
```{R echo = FALSE}
...
ggplot(holddf, aes(x = as.numeric(WeekNum), y = proportion.eaten, color = as.factor(Period))) + geom_point() + facet_wrap(~Trial.x) +
  ...
  scale_color_manual(values = c("1" = "red4",
                                "2"="blue4"))+xlab("Week Number") + ylab("Amount Eaten (Proportionally)") + labs(color = "Period")
...
ggplot(all,aes(x = as.numeric(Trial.x), y = proportion.eaten, color = Species.x)) + geom_point() + geom_smooth(method = "loess")+ 
  ggtitle("Proportion consumed over trial temperature") + scale_y_log10()+
  xlab("Trial Temperature") + 
  ylab("Amount Eaten (Proportionally, log10)")+
  scale_color_manual(values = c("HS" = "darkred",
                                 "CM" = "darkgreen",
                                 "CI" = "darkorange"))+ 
  guides(fill=guide_legend(title="Species"))+ labs(color = "Period")
...
ggplot(all,aes(x = as.numeric(Trial.x), y = proportion.eaten, color = Species.x)) + geom_point() + geom_smooth(method = "loess")+ 
  ...
  coord_cartesian(ylim = c(.001, .15))+ labs(color = "Period")

ggplot(all,aes(x = as.numeric(Trial.x), y = proportion.eaten, color = Species.x)) + geom_point() + geom_smooth(method = "loess")+ 
  ggtitle("Proportion consumed over trial temperature by Period") + scale_y_log10()+
  facet_wrap(~Period)+
  xlab("Trial Temperature") + 
  ylab("Amount Eaten (Proportionally, log10)")+
  scale_color_manual(values = c("HS" = "darkred",
...
```{R echo = FALSE, warning = FALSE}
ggplot(EatDF,aes(x = as.numeric(Trial.x), y = proportion.eaten, color = Species.x)) + geom_point() + geom_smooth(method = "loess")+ 
  ggtitle("Proportion consumed over trial temperature") + scale_y_log10()+
  xlab("Trial Temperature") + 
  ylab("Amount Eaten (Proportionally, log10)")+
  scale_color_manual(values = c("HS" = "darkred",
...
```
```{R, echo = FALSE, warning = FALSE}
ggplot(husbandry, aes(x = Date, y = Temperature, color = Trial)) + geom_point() + facet_wrap(~System)
```