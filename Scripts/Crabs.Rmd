...
library(DHARMa)
all.mod1 <- glmer(amount.eaten ~ Species.x*Temperature*as.factor(Period) + log_mass_bs + log_mass_ws +
                    (1 | runID) + (1 | ID)
                  , family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod1, rank = "AICc")

data$temp_squared <- data$Temperature^2
data$temp_sq_CI <- ifelse(data$Species.x == "CI", data$temp_squared, 0) 
...
# Refit model with centered quadratic
best.mod1 <- glmmTMB(
  amount.eaten ~ Species.x * Temperature + temp_sq_CI + 
                log_mass_ws_sc + log_mass_ws_sc2 + 
                as.factor(Period) + (1 | ID),
...
summary(best.mod1)
sim_res <- simulateResiduals(fittedModel = best.mod1, n = 1000)
plot(sim_res)
...
#best model, out of Gamma(log), and Gaussian - diagnostic plots look "okay"
best.mod1 <- glm(amount.eaten ~ Species.x*Temperature + log_mass_ws, family = gaussian(link = "log"), data = data, na.action = "na.fail")
summary(best.mod1)
sim_res_pos <- simulateResiduals(fittedModel = best.mod1, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
testZeroInflation(sim_res_pos)  # should be low for positive model
testOutliers(sim_res_pos)

#Amount Eaten with log_mass_bs and squared -------------------------------------------------------------------- "OK" model
library(DHARMa)
all.mod <- glm(amount.eaten ~ Species.x*Temperature*as.factor(Period) + log_mass_bs + log_mass_ws + temp_squared, family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod, rank = "AICc")

#best model out of all the families, ranked low on the dredge but still within delta 2. Also diagnostic plots did not look good
best.mod2 <- glm(amount.eaten ~ Species.x*Temperature + log_mass_ws + temp_squared, family = Gamma(link = "identity"), data = data, na.action = "na.fail")
summary(best.mod2)
sim_res_pos <- simulateResiduals(fittedModel = best.mod2, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
testZeroInflation(sim_res_pos)  # should be low for positive model
testOutliers(sim_res_pos)

#Proportion Eaten ---------------------------------------------------------------- OK MODEL

all.mod3 <- glm(proportion.eaten ~ Species.x*Temperature*as.factor(Period) + log_mass_bs + log_mass_ws, family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod3, rank = "AICc")

#best model, compared to Gaussian and link = identity (looks okay...)
best.mod3 <- glm(proportion.eaten ~ Species.x*Temperature + log_mass_ws, family = Gamma(link = "log"), data = data, na.action = "na.fail")
summary(best.mod3)

sim_res_pos <- simulateResiduals(fittedModel = best.mod3, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
testZeroInflation(sim_res_pos)  # should be low for positive model
testOutliers(sim_res_pos)

#Proportion Eaten With Temp Squared---------------------------------------------------------------- OK MODEL

# Hurdle model: zeros vs positives (binomial) + positive consumption (Gamma)
all.mod4 <- glm(proportion.eaten ~ Species.x*Temperature + log_mass_ws + temp_squared, family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod4, rank = "AICc")

#Gamma log and identity look similar in terms of diagonistc plots, plots look "ok"
best.mod4 <- glm(proportion.eaten ~ Species.x*Temperature + temp_squared, family = Gamma(link = "identity"), data = data, na.action = "na.fail")
summary(best.mod4)

sim_res_pos <- simulateResiduals(fittedModel = best.mod4, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
testZeroInflation(sim_res_pos)  # should be low for positive model
testOutliers(sim_res_pos)

#COMPARING THE TOP 4 MODELS------------------------------------------------------

AIC(best.mod1, best.mod2, best.mod3, best.mod4)
#1 and 2 win

summary(best.mod1)
summary(best.mod2)

#PROPORTION EATEN  WITH TEMPERATURE SQUARED ---------------------------------- POOR MODEL DIAGNOSTICS

data$prop_log <- log(data$proportion.eaten)

#Gaussian family diagnostic looks "ok", QQ plot is poor
pos_model <- glm(prop_log ~ as.numeric(Temperature)*Species.x + temp_squared + log_mass_ws, data = data, na.action = "na.fail")
dredge(pos_model)

pos_model <- glm(prop_log ~ as.numeric(Temperature)*Species.x,data = data, na.action = "na.fail")
summary(pos_model)
plot(pos_model)

#SEPARATING THE MODEL BETWEEN ROCK CRABS AND REST MAKES THE ROCK CRAB MODEL LOOK REALLY GOOD, BUT ASIAN SHORE CRAB AND GREEN CRAB FAILS
#PROPORTION EATEN ROCK CRABS WITH TEMPERATUER SQUARED /Log proportion eaten----------------------------------
#-IGNORE THIS SECTION BELOW-------------------------------------------------------------

data2 = subset(finalEatCensor, proportion.eaten > 0 & Species.x == "CI")
data2$log_prop <- log(data2$proportion.eaten)
# Hurdle model: zeros vs positives (binomial) + positive consumption (Gamma)
data2$temp_squared <- data2$Temperature^2

#SOLO ROCK CRAB MODEL log proportion - best preforming model
pos_model2_rock <- glm(log_prop ~ as.numeric(Temperature) + temp_squared, data = data2, na.action = "na.fail")
summary(pos_model2_rock)
plot(pos_model2_rock)
testDispersion(pos_model2_rock)

#proportion eaten, best looking model diagnostic plot wise
pos_model3 <- glm(proportion.eaten ~ as.numeric(Temperature) + temp_squared, family = Gamma(link = "identity"), data = data2, na.action = "na.fail")
summary(pos_model3)
sim_res_pos <- simulateResiduals(fittedModel = pos_model3, n = 1000)
plot(sim_res_pos)

AIC(pos_model2_rock, pos_model3)

#PROPORTION EATEN shore and green CRABS WITH TEMPERATUER SQUARED /Log proportion eaten----------------------------------

data2 = subset(finalEatCensor, proportion.eaten > 0 & (Species.x == "CM" | Species.x == "HS"))
data2$log_prop <- log(data2$proportion.eaten)
# Hurdle model: zeros vs positives (binomial) + positive consumption (Gamma)
data2$temp_squared <- data2$Temperature^2

#removed temperature squared - best preforming model
pos_model2_rest <- glm(log_prop ~ as.numeric(Temperature)*Species.x, data = data2, na.action = "na.fail")
summary(pos_model2_rest)
plot(pos_model2_rest)
testDispersion(pos_model2_rest)

#proportion eaten, diagnostic plot fits!!!
pos_model3 <- glm(proportion.eaten ~ as.numeric(Temperature)*Species.x, family = Gamma(link = "log"), data = data2, na.action = "na.fail")
summary(pos_model3)
sim_res_pos <- simulateResiduals(fittedModel = pos_model3, n = 1000)
plot(sim_res_pos)
testDispersion(pos_model3)

#pos_model 2 is the best
AIC(pos_model2_rest, pos_model3)
#-IGNORE THIS SECTION ABOVE-------------------------------------------------------------
#graphing the model-----------------------------------------------
#--------------------------------------

#best model, let's try splitting it between rock and asian/green crab
library(DHARMa)
all.mod1 <- glm(amount.eaten ~ Species.x*Temperature*as.factor(Period) + log_mass_bs + log_mass_ws, family = Gamma(link = "log"), data = data, na.action = "na.fail")
dredge(all.mod1, rank = "AICc")

#best model, out of Gamma(log), and Gaussian - diagnostic plots look "okay"
best.mod1 <- glm(amount.eaten ~ Species.x*Temperature + log_mass_ws, family = Gamma(link = "identity"), data = data, na.action = "na.fail")
summary(best.mod1)
sim_res_pos <- simulateResiduals(fittedModel = best.mod1, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
...
data2 = subset(finalEatCensor, proportion.eaten > 0 & Species.x == "CI")
data2$temp_c <- scale(data2$Temperature, center = TRUE, scale = FALSE)
data2$temp_squared <- data2$temp_c^2
all.mod2 <- glm(amount.eaten ~ temp_c*as.factor(Period)*temp_squared + log_mass_ws, family = Gamma(link = "log"), data = data2, na.action = "na.fail")
dredge(all.mod2, rank = "AICc")

#model 2
best.mod2 <- glm(amount.eaten ~ temp_c+ temp_squared+ log_mass_ws, family = Gamma(link = "inverse"), data = data2, na.action = "na.fail")
summary(best.mod2)
sim_res_pos <- simulateResiduals(fittedModel = best.mod2, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
...
data2$temp_c <- as.numeric(scale(data2$Temperature, center = TRUE, scale = FALSE))
data2$temp_squared <- data2$temp_c^2

#model 3
best.mod3 <- glm(amount.eaten ~ temp_c*temp_squared+ log_mass_ws, family = Gamma(link = "inverse"), data = data2, na.action = "na.fail")
summary(best.mod3)
sim_res_pos <- simulateResiduals(fittedModel = best.mod2, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
...
AIC(best.mod2, best.mod3)

#ASIAN SHORE CRAB AND GREEN CRAB ONLY

data2 = subset(finalEatCensor, proportion.eaten > 0 & (Species.x == "CM" | Species.x == "HS"))

#best model, out of Gamma(log), and Gaussian - diagnostic plots look "okay"
best.mod1 <- glm(amount.eaten ~ Species.x*Temperature + log_mass_ws, family = Gamma(link = "identity"), data = data2, na.action = "na.fail")
summary(best.mod1)
sim_res_pos <- simulateResiduals(fittedModel = best.mod1, n = 1000)
plot(sim_res_pos) # residuals vs fitted values
testDispersion(sim_res_pos)
...
```{R}
# Build a separate prediction grid at exact temperatures
pred_exact <- expand.grid(
  Temperature = c(18, 20, 22, 24, 26),
  Species.x = c("CI", "CM", "HS")
) %>%
  mutate(
    temp_sq_CI      = ifelse(Species.x == "CI", 
                             (Temperature - mean_temp)^2, 0),
    log_mass_ws_sc  = 0,
    log_mass_ws_sc2 = 0,
    Period          = factor(1, levels = c(1, 2)),
    ID              = NA
  )
...
# Build model matrix and get SEs
mm_exact <- model.matrix(~ Species.x * Temperature + temp_sq_CI + 
                           log_mass_ws_sc + log_mass_ws_sc2 +
                           as.factor(Period),
                         data = pred_exact)
...
# View results with ratios on original scale
contrast_df %>%
  mutate(
...
# Rebuild prediction grid with centered quadratic
pred_df <- expand.grid(
  Temperature = temp_seq,
  Species.x = c("CI", "CM", "HS")
) %>%
...
# Build model matrix
mm <- model.matrix(~ Species.x * Temperature + temp_sq_CI + 
                     log_mass_ws_sc + log_mass_ws_sc2 +
                     as.factor(Period),
                   data = pred_df)
...
# Check CI SE values now
pred_df %>%
  filter(Species.x == "CI") %>%
  select(Temperature, fit, lwr, upr, se_link)

# Plot on log scale to handle wide CIs
ggplot() +
