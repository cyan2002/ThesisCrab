...
holddf <- subset(all, all$Species.x == "CI")
ggplot(holddf, aes(x = WeekNum, y = proportion.eaten, color = as.factor(Period))) + geom_point() + facet_wrap(~Trial.x) +
  ggtitle("Period 1+2, Cancer Irroratus") + geom_smooth(method = "loess")

#new column that is the temperature difference from it's target temperature
all$tempDiff <- abs(all$Trial.x - all$Temperature)
...
```{R}
#Only the 16C trial has 8 data points, the rest have 9 making it easier to calculate by week. 
...
#used to loop through each specific trial by species
preload_spp <- c("CI", "CM", "HS")
preload_tr <- c("26", "24", "22", "20", "18")

for(a in 1:length(preload_spp)){
  for(b in 1:length(preload_tr) ){
    for(i in 1:2){
      #creating a dataframe that subsets the exact trial by species we want to collect average from
      holddf <- subset(all, all$Species.x == preload_spp[a] & all$Trial.x == preload_tr[b] & all$Period == i)
      
      #Collecting all the dates from the trial
      holddf$Date.x <- as.Date(holddf$Date.x)
      
      #Sorting the dates in order
      dates <- sort(unique(holddf$Date.x))
      
      #Taking the last 3 dates which would be the trial days or 3rd week
      last.dates <- tail(dates, 3)
      
      #Changing the dataframe so that it only contains data from the trial week 
      holddf <- subset(holddf, holddf$Date.x == last.dates[1] | holddf$Date.x == last.dates[2] | holddf$Date.x == last.dates[3])
      
      #adding these values to the empty lists for week 3
      ...
      obs_count <- append(obs_count, nrow(na.omit(holddf)))
      week_list <- append(week_list, 3)
    }
  }
}

average_df <- data.frame(avg_list, species_list, trial_list, period_list, obs_count)
