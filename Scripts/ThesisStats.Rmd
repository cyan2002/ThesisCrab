...
RUN THIS FOR 1: sensoring the data
...
RUN THIS FOR 2: replacement data is in
...
MAKE THOSE WHO Didn't eat TO 0 (AS OPPOSED TO JUST LEAVING IT) and making values that are negative, 0 or positive
```{R}
...
all$proportion.eaten <- ifelse(grepl("DE", all$Notes.x, fixed = TRUE), 0.01, all$proportion.eaten)
all$proportion.eaten <- ifelse(all$proportion.eaten < 0, 0.01, all$proportion.eaten)
all$prop_log <- log(all$proportion.eaten / (1 - all$proportion.eaten))
hist(all$prop_log)
...
#subsetting by species Gamma-------------------------------------------------------
library(lme4)

holddf <- subset(all, all$Species.x == "CI")
all.mod <- glm(proportion.eaten ~ Period*Trial.x, family = Gamma(link = "inverse"), data = holddf, na.action = "na.fail")
dredge(all.mod)
best.mod <- glmer(proportion.eaten ~ Trial.x + (1|Period), family = Gamma(link = "inverse"), data = holddf)
hist(residuals(best.mod, type = "pearson"))
summary(best.mod)
sim <- simulateResiduals(best.mod)
plot(sim)
...
all.mod <- glm(proportion.eaten ~ Period*Trial.x, family = Gamma(link = "inverse"), data = holddf, na.action = "na.fail")
dredge(all.mod)
best.mod <- glm(proportion.eaten ~ 1, family = Gamma(link = "inverse"), data = holddf)
hist(best.mod$residuals)
summary(best.mod)
sim <- simulateResiduals(best.mod)
plot(sim)
...
all.mod <- glm(proportion.eaten ~ Period*Trial.x, family = Gamma(link = "inverse"), data = holddf, na.action = "na.fail")
dredge(all.mod)
best.mod <- glm(proportion.eaten ~ Period*Trial.x, family = Gamma(link = "inverse"), data = holddf)
hist(best.mod$residuals)
summary(best.mod)
sim <- simulateResiduals(best.mod)
plot(sim)

#subsetting by species using proportion eaten----------------------------------------
#wait, suprisingly really good...?
holddf <- subset(all, all$Species.x == "CI")
all.mod <- glm(proportion.eaten ~ Period*Trial.x, data = holddf, na.action = "na.fail")
dredge(all.mod)
best.mod <- glm(proportion.eaten ~ Period + Trial.x, data = holddf, na.action = "na.fail")
hist(best.mod$residuals)
...
#adding all species into one model fails
all.mod <- glm(proportion.eaten ~ Period*Trial.x*Species.x, data = all, na.action = "na.fail")
dredge(all.mod)
best.mod <- glm(proportion.eaten ~ Period*Trial.x*Species.x, data = all)
hist(best.mod$residuals)
par(mfrow=c(2,2)) # Change the panel layout to 2 x 2
plot(best.mod)

#-------------------------------------------------------------
#proportion logged subsetted by species
holddf <- subset(all, all$Species.x == "CI")
all.mod <- glm(prop_log ~ Period*Trial.x, data = holddf, na.action = "na.fail")
dredge(all.mod)
best.mod <- glm(proportion.eaten ~ Trial.x, data = holddf)
hist(best.mod$residuals)
par(mfrow=c(2,2)) # Change the panel layout to 2 x 2
plot(best.mod)

emm <- emmeans(best.mod, ~ Trial.x)
emm_cld <- multcomp::cld(emm, Letters = letters)
emm_cld_df <- as.data.frame(emm_cld)
emm_cld_df$emmean <- exp(emm_cld_df$emmean) - 1
emm_cld_df$lower.CL <- exp(emm_cld_df$lower.CL) - 1
emm_cld_df$upper.CL <- exp(emm_cld_df$upper.CL) - 1

#unsure how to graph if null model is the best...?
ggplot(emm_cld_df, aes(x = Trial.x, y = emmean, fill = Trial.x)) +
  ...
  geom_text(aes(label = .group),
            ...
            vjust = -0.5) +
  labs(y = "Propotion Consumed", x = "Trial") +
  theme_minimal()+
  scale_fill_manual(values = c("18" = "darkblue",
                                "20"="darkgreen",
                                "22"="darkorange",
                               "24" = "red3",
                               "26" = "salmon"))+
  ggtitle("Proportion consumed across Trial for Cancer")
...
ggplot(emm_cld_df, aes(x = Trial.x, y = emmean, fill = Trial.x)) +
  ...
  scale_fill_manual(values = c("18" = "darkblue",
                               ...
                               "24" = "red3",
                               "26" = "salmon"))+
  ggtitle("Proportion consumed across Trial for Carcinus")

#same outliers from HS before...
holddf <- subset(all, all$Species.x == "HS")
all.mod <- glm(prop_log ~ Period*Trial.x, data = holddf, na.action = "na.fail")
dredge(all.mod)
best.mod <- glm(proportion.eaten ~ Trial.x, data = holddf)
...
ggplot(emm_cld_df, aes(x = Trial.x, y = emmean, fill = Trial.x)) +
  ...
  scale_fill_manual(values = c("18" = "darkblue",
                               ...
                               "24" = "red3",
                               "26" = "salmon"))+
  ggtitle("Proportion consumed across Trial for Hemigrapsus")

#proportion logged fails with all species
mod <- glmer(prop_log ~ Period + Trial.x + Species.x + Period*Trial.x*Species.x + (1 | Species.x), data=all, na.action = "na.fail")
dredge(mod)
best.mod <- glm(prop_log ~ Species.x*Trial.x, data = all)
hist(best.mod$residuals)
plot(best.mod)
summary(best.mod)
#--------------------------

emm <- emmeans(best.mod, ~ Species.x * Trial.x)

emm_cld <- multcomp::cld(emm, Letters = letters)
emm_cld_df <- as.data.frame(emm_cld)

emm_cld_df$emmean <- exp(emm_cld_df$emmean)
#emm_cld_df$SE <- exp(emm_cld_df$SE)
emm_cld_df$lower.CL <- exp(emm_cld_df$lower.CL)
emm_cld_df$upper.CL <- exp(emm_cld_df$upper.CL)

ggplot(emm_cld_df, aes(x = Species.x, y = emmean, fill = Trial.x)) +
  geom_col(position = position_dodge(0.9)) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL),
                width = 0.2, position = position_dodge(0.9)) +
  geom_text(aes(label = .group),
            position = position_dodge(0.9),
            vjust = -6) +
  labs(y = "Propotion Consumed", x = "Species") +
  theme_minimal()+
  scale_fill_manual(values = c("18" = "darkblue",
                                "20"="darkgreen",
                                "22"="darkorange",
                               "24" = "red3",
                               "26" = "salmon"))+
  ggtitle("Proportion consumed across Species and Trial")


  

#END - REST ARE QUESTIONS FOR BRIAN
#proportion logged diagnostic plots look great across all species; however, different models (including different dependent varaibles were used). Additionally, when putting all species into a single model, it fails 
#oddly enough, using proportion consumed (not logged) across typical GLMs (no gamma) also worked well and diagnostic plots seems good
...
-What is the difference between DHARMa qq plots vs the regulat qqplots?


Notes
-Trial is replaced with temperature in Crab Consumption data
-Explore WW, Trial(temperature), Species matters for consumption and death
-Temperature as a continous predictor, temperature by species (does temperature predict death ignoring species)
-WW predictive towards death
-Bias reduced logistic regression (helps against separation problem)
-Don't jump straight into models - determine what needs to be continous, factor etc
-show/explore the initial data, X number of individuals, this number of x crabs from x species etc
-Word Doc not an HTML
-cleanest way to handle data - sensor the data, remove those that are dead and replaced
-just compare and model the two most plausible ones (the cleanest versions or extreme versions: sensor data, data died stays in and replacement data stays in), model with the most inclusive and conservative
-run two different models, no need to over complicate